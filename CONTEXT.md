...
**Principal search**:
A transient, session-only text filter that narrows the Roster tree to repos containing at least one Principal whose label case-insensitively substring-matches the typed text — checked across Direct, Group, and Member entries alike. AND-combined with the active view mode (All access / Changes only) and with **Excluded principal**: a row must pass all conditions to stay visible, and a repo disappears once none of its rows do. A repo kept visible by a match auto-expands, overriding the diff-based default-open rule. Within a repo that stays visible, only the rows that themselves match the search render — an active search narrows row-by-row, not just repo-by-repo (revised after PD-85's original "full context" behavior read as a bug in practice: PD-86 tightened it). Not persisted across app restarts and not staged against any Snapshot — it survives Baseline/Comparison navigation like view mode does, and is cleared only by the existing Reset control. Deliberately distinct from **Excluded principal**: search narrows what's shown by a positive match, exclusion suppresses by a standing negative rule — the two compose rather than replace one another.
...
**Excluded principal**:
A Principal (user or group) the auditor has chosen to hide from the Roster tree for the rest of the current session — added via a "Hide" link on the Principal's own row, matched by stable `principal.id`, never by label text (a deliberate departure from Principal search's substring-on-label matching). Hiding a group also hides every `Member` entry derived from it (matched by `accessType.group_id`), but never an unrelated Direct or other Group entry the same person separately holds — each `PrincipalEntry` is matched independently (ADR-0032, `docs/adr/0032-excluded-principal-group-cascade-scoped-to-member-rows.md`). Composes as a third AND-ed condition alongside view mode and Principal search: a repo disappears once none of its rows pass all three, and the roster's live/changed/rows counts recompute over exactly what's left. Session-scoped only, matching Principal search's lifecycle exactly — survives Baseline/Comparison navigation, cleared by the existing Reset control, never persisted across restarts or staged against any Snapshot. Never respected by Export, which stays a flat, unfiltered, diff-free roster regardless of on-screen state (see Export). Tracked and managed via a slim banner atop the tree ("N principals hidden this session") that expands into a per-item "Unhide" list plus a bulk "Unhide all".
_Avoid_: Suppressed principal, Hidden principal (the row action and banner copy say "Hide"/"hidden" for the user-facing verb, but this glossary's canonical noun is Excluded principal), Filter (see Principal search's _Avoid_ note — same reasoning)
