...
**Principal search**:
A transient, session-only text filter that narrows the Roster tree to repos containing at least one Principal whose label case-insensitively substring-matches the typed text — checked across Direct, Group, and Member entries alike. AND-combined with the active view mode (All access / Changes only): a row must satisfy both to stay visible, and a repo disappears once none of its rows do. A repo kept visible by a match auto-expands, overriding the diff-based default-open rule. Within a repo that stays visible, only the rows that themselves match the search render — an active search narrows row-by-row, not just repo-by-repo (revised after PD-85's original "full context" behavior read as a bug in practice: PD-86 tightened it). Not persisted across app restarts and not staged against any Snapshot — it survives Baseline/Comparison navigation like view mode does, and is cleared only by the existing Reset control. Deliberately distinct from the not-yet-built **Excluded principal** (a reverse filter permanently hiding a specific noisy Principal, e.g. an Auditors group — tracked separately, out of scope here): search narrows what's shown by a positive match, exclusion suppresses by standing negative rule.
_Avoid_: Filter (too generic — already loosely associated with the existing All access/Changes only view mode, which the code itself calls `ViewMode`, not a filter)

**Pending edit**:
A staged Level-change or Remove grant against a Direct or Group grant, held only in local UI state until Apply — never partial, never sent automatically. Optionally carries a Justification, typed at staging time. Only possible while the Comparison is the latest Snapshot; discarded outright (not carried forward) if a new Run or Refresh produces a Snapshot that supersedes the one it was staged against (ADR-0022, `docs/adr/0022-permission-edits-staged-batch-apply.md`) — a Refresh supersedes the source Snapshot exactly as a Run does, so the same clearing rule applies to whichever staged edits are left over (e.g. ones that failed to apply).
...
**Justification**:
An optional free-text note the user can attach to a single Pending edit, at staging time, recording why a Direct or Group grant is being changed or removed. Never sent to Bitbucket — the permissions-config API has no field for it — so it exists purely as local audit context, persisted to the Applied-edit log only if and when that specific edit is later applied successfully. Not required: an edit can be staged, and applied, with no justification at all.
_Avoid_: Reason code (implies a fixed taxonomy; this is free text), comment

**Applied-edit log**:
A local, append-only record of every Pending edit that has actually been applied to Bitbucket, written at the moment `apply_pending_edits` reports success for that edit — never for a failed attempt, which stays staged for retry and is logged only if a later attempt succeeds. Each row carries the edit's repo/project, Principal, scope, action, before/after permission, its Justification (if any), the Apply timestamp, and the `account` string from the stored Credentials' `username` field (not a verified identity — this tool has no auth model). Deliberately carries no reference to the Snapshot the edit was staged against: a Snapshot is freely deletable at any time (see Snapshot) as routine cleanup unrelated to whether an edit staged against it was ever applied, so tying the log to a Snapshot id would either dangle or cascade-delete the very audit trail this entity exists to preserve (ADR-0032, `docs/adr/0032-applied-edit-log-decoupled-from-snapshot.md`).
_Avoid_: Audit log (this tool is itself a permissions audit tool end-to-end; "Applied-edit log" names this one specific record of live mutations, not the whole app's purpose), history
